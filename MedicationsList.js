...
const MedicationSchedule = ({ user }) => {
  ...
  const fetchPrescriptions = async (user) => {
    try {
      ...
      if (response.data.prescriptions && response.data.prescriptions.length > 0) {
        ...
        let tempMedications = [];
        let tempMarkedDates = {};
        let colorOpacity = 1;  // Start with full opacity for the first prescription
  
        // Loop through each prescription
        response.data.prescriptions.forEach((latestPrescription, prescriptionIndex) => {
          ...
          latestPrescription.medicines.forEach((medicine, index) => {
            const medicineColor = getColor(index, colorOpacity); // Get color with current opacity
          ...
          // Reduce opacity for each subsequent prescription
          colorOpacity = Math.max(0, colorOpacity - 0.2);
        });
  ...
  const getColor = (index, opacity) => {
    ...
    return changeColorOpacity(color, opacity); // Apply the opacity to the color
  };
  
  // Function to change the color's opacity
  const changeColorOpacity = (color, opacity) => {
    return color + Math.round(opacity * 255).toString(16).padStart(2, '0'); // Adds the opacity as an alpha channel (rgba)
  };
